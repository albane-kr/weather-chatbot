...
class TemperatureDataset(Dataset):
    def __init__(self, temp_max_folder, temp_min_folder, sequence_length=7, prediction_length=3):
        self.data = []
        ...
        for file in tqdm(os.listdir(temp_max_folder)):
            if file.endswith(".csv"):
                ...
                df = pd.read_csv(os.path.join(temp_max_folder, file))
                df_2 = pd.read_csv(os.path.join(temp_min_folder, file))
                df['temp_min'] = df_2['temp_min']
